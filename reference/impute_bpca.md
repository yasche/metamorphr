...
`pcaMethods::`[`pca`](https://rdrr.io/pkg/pcaMethods/man/pca.html)`(method = "bpca")`.
...
`impute_bpca()` depends on the `pcaMethods` package from Bioconductor.
If `metamorphr` was installed via
[`install.packages()`](https://rdrr.io/r/utils/install.packages.html),
dependencies from Bioconductor were not automatically installed. When
`impute_bpca()` is called without the `pcaMethods` package installed,
you should be asked if you want to install `pak` and `pcaMethods`. If
you want to use `impute_bpca()` you have to install those. In case you
