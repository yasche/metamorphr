...
One of several PCA-based imputation methods. Basically a wrapper around
`pcaMethods::`[`pca`](https://rdrr.io/pkg/pcaMethods/man/pca.html)`(method = "ppca")`.
...
(`pcaMethods::`[`pca`](https://rdrr.io/pkg/pcaMethods/man/pca.html)`(method = "ppca")`),
the order of columns has an influence on the outcome. Therefore, calling
`pcaMethods::`[`pca`](https://rdrr.io/pkg/pcaMethods/man/pca.html)`(method = "ppca")`
on a matrix and calling `metamorphr::impute()` on a tidy tibble might
give different results, even though they contain the same data. That is
because under the hood, the tibble is transformed to a matrix prior to
calling
`pcaMethods::`[`pca`](https://rdrr.io/pkg/pcaMethods/man/pca.html)`(method = "ppca")`
and you have limited influence on the column order of the resulting
matrix.

*Important Note*

`impute_ppca()` depends on the `pcaMethods` package from Bioconductor.
If `metamorphr` was installed via
[`install.packages()`](https://rdrr.io/r/utils/install.packages.html),
dependencies from Bioconductor were not automatically installed. When
`impute_ppca()` is called without the `pcaMethods` package installed,
you should be asked if you want to install `pak` and `pcaMethods`. If
you want to use `impute_ppca()` you have to install those. In case you
run into trouble with the automatic installation, please install
`pcaMethods` manually. See [pcaMethods – a Bioconductor package
providing PCA methods for incomplete
...
- scale:
  ...
  Should `data` be scaled? See
  [`prep`](https://rdrr.io/pkg/pcaMethods/man/prep.html) for details.

- direction:

  Either `1` or `2`. `1` runs a PCA on a matrix with samples in columns
  and features in rows and `2` runs a PCA on a matrix with features in
  columns and samples in rows. Both are valid according to this
  [discussion on
  GitHub](https://github.com/hredestig/pcaMethods/issues/25) but give
  **different results**.
